...
async function executeScript(params: {
  ...
  // Use the dedicated virtual environment for Job Agent
  const pythonCommand = process.platform === 'win32' ? 
    path.join(process.cwd(), 'JobAgent', 'venv', 'Scripts', 'python.exe') : 
    path.join(process.cwd(), 'JobAgent', 'venv', 'bin', 'python');
